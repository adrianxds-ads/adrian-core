...
.ad-close{font-size:16px;min-width:40px}.ad-keyboard-preview{min-width:0;flex:1;padding:0 7px;color:#b9c8cb;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;font-size:12px}.ad-keyboard-preview b{color:#f7fafb;font-size:14px}
...
.ad-key{min-width:0;min-height:54px;border:1px solid rgba(255,255,255,.17);border-radius:10px;color:#fff;font:950 30px/1 system-ui;box-shadow:inset 0 1px rgba(255,255,255,.08),inset 0 -3px rgba(0,0,0,.19);touch-action:manipulation;-webkit-user-select:none;user-select:none}
...
function updatePreview(){const el=root?.querySelector('.ad-keyboard-preview');if(!el)return;const value=String(active?.value??'');el.innerHTML='<span>ESCRIBIENDO · </span><b>'+escapeHtml(value.slice(-24)||'…')+'</b>';}
...
function render(){
 ...
 el.innerHTML='<div class="ad-keyboard-inner"><div class="ad-keyboard-top"><div class="ad-keyboard-langs">'+['es','ca','en'].map(x=>'<button type="button" class="ad-lang '+(x===lang?'active':'')+'" data-lang="'+x+'">'+x.toUpperCase()+'</button>').join('')+'</div><div class="ad-keyboard-preview"></div><button type="button" class="ad-close" data-action="close" aria-label="Cerrar teclado">×</button></div>'+
 rows.map((r,i)=>rowHtml(r,i,rowClasses[i])).join('')+
 '<div class="ad-keyboard-controls"><button class="ad-key control mode" type="button" data-action="mode">'+(mode==='alpha'?'123':'ABC')+'</button><button class="ad-key control question" type="button" data-char="¿">¿</button><button class="ad-key control space" type="button" data-action="space">ESPACIO</button><button class="ad-key control question" type="button" data-char="?">?</button><button class="ad-key control back" type="button" data-action="back" aria-label="Borrar">⌫</button></div></div>';
...
function keepActiveVisible(smooth=false){
 ...
 const topGuard=Math.max(54,document.querySelector('.adrian-tools')?.getBoundingClientRect().bottom||0)+8,bottomLimit=kbTop-10;
...
function syncHeight(){requestAnimationFrame(()=>{if(!root)return;document.documentElement.style.setProperty('--ad-keyboard-height',root.getBoundingClientRect().height+'px');settleVisibility();});}
...
function enhance(scope=document){
 scope.querySelectorAll?.('[data-ad-keyboard]').forEach(el=>{
  ...
  el.addEventListener('pointerdown',()=>setTimeout(()=>open(el),0));
 });
}
