...
function open(el){active=el;lang=el.dataset.adKeyboard||'en';el.setAttribute('inputmode','none');el.setAttribute('autocomplete','off');el.setAttribute('autocorrect','off');el.setAttribute('autocapitalize','none');el.setAttribute('spellcheck','false');render();root.classList.add('open');document.body.classList.add('ad-keyboard-open');requestAnimationFrame(()=>{document.documentElement.style.setProperty('--ad-keyboard-height',root.getBoundingClientRect().height+'px');el.scrollIntoView({block:'center',behavior:'smooth'});});el.focus({preventScroll:true});}
